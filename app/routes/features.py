...
def export_resume_pdf():
    ...
    if not data or 'html' not in data:
        return jsonify({'error': 'No HTML payload provided.'}), 400
...
<head>
    ...
    <style>
        ...
        * {{
        ...
        /* Attempt to keep sections together */
        .resume-section {{
            page-break-inside: avoid;
        }}
        
        /* Hide buttons/icons that shouldn't be printed */
        .fa-solid, .fa-regular, .fa-brands {{ display: none !important; }}
    </style>
</head>
<body>
    <div class="resume-sheet {theme_val}" id="resume-sheet-paper">
        {resume_html}
    </div>
</body>
</html>"""
    ...
    filename = (data.get('name', 'Resume').replace(' ', '_')) + '_ATS_Resume.pdf'
...
@features_bp.route('/resume-analyzer/export-pdf-debug', methods=['POST'])
@login_required
def export_resume_pdf_debug():
    try:
        from weasyprint import HTML, CSS
        from weasyprint.text.fonts import FontConfiguration
    except Exception as e:
    ...
    data = request.get_json()
    if not data or 'html' not in data:
        return jsonify({'error': 'No HTML payload provided.', 'validation_errors': ['Missing HTML payload']}), 400
    ...
    # Validation
    val_errors = []
    if not data.get('name'): val_errors.append("Missing Name")
    
    full_html = f"""<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Resume Debug</title>
    <style>
        @page {{ size: A4 portrait; margin: 10mm 12mm; }}
        body {{ font-family: Arial, Helvetica, Calibri, sans-serif !important; font-size: 11pt; line-height: 1.3; color: #000; background: #fff; margin: 0; padding: 0; }}
        * {{ box-shadow: none !important; background-color: transparent !important; border-radius: 0 !important; }}
        .resume-render-header {{ border-bottom: 1px solid #000 !important; margin-bottom: 10px !important; padding-bottom: 5px !important; }}
        .section-title, h2, h3 {{ border-bottom: 1px solid #000 !important; text-transform: uppercase !important; font-size: 12pt !important; font-weight: bold !important; margin-top: 12px !important; margin-bottom: 6px !important; padding-bottom: 2px !important; color: #000 !important; }}
        p, ul, ol, div {{ margin-top: 0; margin-bottom: 4px; }}
...
<body>
    ...
    try:
        font_config = FontConfiguration()
        HTML(string=full_html).write_pdf(font_config=font_config)
    except Exception as e:
        return jsonify({
            'status': 'error',
            'weasyprint_errors': str(e),
